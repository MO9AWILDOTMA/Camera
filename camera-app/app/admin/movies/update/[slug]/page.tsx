...
const UpdateMovie = () => {
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      try {
        const response = await moviesApi.getById(slug as string);
        const movie = response.data;

        setInitialData({
          ...movie,
          releaseDate: movie.releaseDate ? parseISO(movie.releaseDate) : null,
        });
  ...
  const handleSubmit = async (data: any) => {
    ...
    try {
      const response = await moviesApi.update(initialData.id, movie);
      ...
      toast({
        title: "Update failed",
        description: error.response?.data?.message || "Movie update failed",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="container mx-auto px-4">
      <div className="flex items-center justify-between ">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Update Movie</h1>
        <Button
          variant="outline"
          onClick={() => router.back()}
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-1/2 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Upload additional posters and screenshots (multiple allowed)
            </p>
          </div>
        </div>
      </div>
